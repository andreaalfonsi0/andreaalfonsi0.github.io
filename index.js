...
window.onscroll = function() {
  ...
  setTimeout(function() {
    ...
    // path.style.strokeDashoffset = Math.round(- length * perc);
  }, 100);
}

window.onresize = function() {
  let path = createPlanePath().path;
  document.querySelector(`#${PAPER_PATH_ID}`).setAttribute("d", path)
  document.querySelector(`#${DASHED_PATH_ID}`).setAttribute("d", path)
}
